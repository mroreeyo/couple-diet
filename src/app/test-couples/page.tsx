      ...
      case 'none':
        return (
          <div className="bg-gray-100 p-4 rounded-lg">
            <h3 className="text-lg font-semibold mb-2">커플 관계 없음</h3>
            <p className="text-gray-600 mb-4">현재 커플 관계가 없습니다. 새로운 커플 요청을 보내보세요!</p>
          </div>
        )
      ...
      case 'pending_sent':
        return (
          <div className="bg-yellow-100 p-4 rounded-lg">
            <h3 className="text-lg font-semibold mb-2">커플 요청 전송됨</h3>
            <p className="text-gray-600 mb-2">다음 사용자에게 커플 요청을 보냈습니다:</p>
            {partnerInfo && (
              <div className="bg-white p-2 rounded">
                <p><strong>이메일:</strong> {partnerInfo.email}</p>
                <p><strong>이름:</strong> {partnerInfo.displayName}</p>
              </div>
            )}
            {requestInfo && (
      ...
      case 'pending_received':
        return (
          <div className="bg-blue-100 p-4 rounded-lg">
            <h3 className="text-lg font-semibold mb-2">커플 요청 받음</h3>
            <p className="text-gray-600 mb-2">다음 사용자로부터 커플 요청을 받았습니다:</p>
            {partnerInfo && (
      ...
      case 'active':
        return (
          <div className="bg-green-100 p-4 rounded-lg">
            <h3 className="text-lg font-semibold mb-2">🎉 커플 관계 활성화됨</h3>
            <p className="text-gray-600 mb-2">현재 파트너:</p>
            {partnerInfo && (
              <div className="bg-white p-2 rounded mb-4">
                <p><strong>이메일:</strong> {partnerInfo.email}</p>
                <p><strong>이름:</strong> {partnerInfo.displayName}</p>
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <h2 className="text-xl font-semibold mb-4">1. 인증 토큰 설정</h2>
          <p className="text-gray-600 mb-4">
            먼저 <a href="/test-api-endpoints" className="text-blue-500 underline">API 테스트 페이지</a>에서 
            로그인하여 토큰을 받아오세요.
          </p>
          <input
            type="text"
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-semibold">2. 현재 커플 상태</h2>
            <button
          ...
          {coupleStatus ? renderCoupleStatusUI() : (
            <p className="text-gray-500">토큰을 입력하고 상태를 확인해주세요.</p>
          )}
        </div>

        {/* 커플 요청 보내기 */}
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <h2 className="text-xl font-semibold mb-4">3. 커플 요청 보내기</h2>
          <div className="flex gap-2">
            <input
              type="email"
              value={partnerEmail}
        ...
        {/* 수동 요청 응답 */}
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <h2 className="text-xl font-semibold mb-4">4. 수동 요청 응답 (고급)</h2>
          <p className="text-gray-600 mb-4">특정 요청 ID로 직접 응답하려면 사용하세요.</p>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-xl font-semibold mb-4">5. 마지막 API 응답</h2>
          {lastResponse ? (
            <div className={`p-4 rounded-lg ${lastResponse.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
              ...
              {lastResponse.message && (
                <p className="text-gray-700 mb-2">{lastResponse.message}</p>
              )}
              {lastResponse.error && (
            ...
            <p className="text-gray-500">아직 API 요청을 하지 않았습니다.</p>
          )}
        </div>

        {/* 사용 안내 */}
        <div className="bg-blue-50 rounded-lg p-6 mt-6">
          <h3 className="text-lg font-semibold mb-2">📝 사용 안내</h3>
          <ol className="list-decimal list-inside space-y-1 text-gray-700">
            <li>먼저 <a href="/test-api-endpoints" className="text-blue-500 underline">API 테스트 페이지</a>에서 로그인하여 토큰을 받아옵니다.</li>
